...
void main() {
    ...
    for(int i=0; i< 3; i++) {
        vec3 lightDirection = -normalize(varyingPosition-lights[i].lightDirection);
        float diffuse = max(0.0, dot(textureNormal, lightDirection));
        float attenuation = attenuate(distance(varyingPosition, lights[i].lightDirection)/ 6.0, 0.01, 0.05);
